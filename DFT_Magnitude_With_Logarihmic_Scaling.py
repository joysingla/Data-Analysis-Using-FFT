...
# Below 2 lines are for converting the magnitude obtained by DFT formula into dB  
mag = np.abs(dft_result)
...
plt.plot(freqs, scaled_magnitude, label='Complete Magnitude')  # 'label' parameter defines the name that will be displayed on the graph 
plt.title('DFT Analysis for Magnitude')
plt.xlabel('Frequency (Hz)')
plt.ylabel('DFT Result (Complete Magnitude)')
plt.grid(True)
plt.legend()
plt.show()
